...
async fn handle_command(
    ...
    match cmd {
        ...
        "get_theme_settings" => {
            let theme = executor::execute_tmux_command(&[
                "show-options", "-gqv", "@tmuxy-theme",
            ])
            .map(|s| s.trim().to_string())
            .unwrap_or_default();
            let mode = executor::execute_tmux_command(&[
                "show-options", "-gqv", "@tmuxy-theme-mode",
            ])
            .map(|s| s.trim().to_string())
            .unwrap_or_default();
            Ok(serde_json::json!({
