#!/bin/bash
# Navigate to the previous tab in a pane group
...
# Find index of visible pane and compute previous
IDX=0
VISIBLE_IDX=0
...
if [ "$VISIBLE_IDX" -gt 0 ]; then
  PREV_IDX=$((VISIBLE_IDX - 1))
else
  PREV_IDX=$((PANE_COUNT - 1))
fi

TARGET=$(echo "$GRP_JSON" | jq -r --arg gid "$GROUP_ID" --argjson idx "$PREV_IDX" '.groups[$gid].paneIds[$idx]')
