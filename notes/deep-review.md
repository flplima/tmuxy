...
5. **[~] PARTIALLY FIXED — the ghost-pane half is done; the rebuild half is BLOCKED (do not retry naively).** The sniff now requires the `%<digits>,` shape tmux actually emits, so arbitrary `RunCommand` output can no longer conjure ghost panes (commit `078cf5d`). The `ChangeType::None`-for-empty-acks half was implemented, shipped, and then **reverted** (commit `2ad03a3`): the frontend's optimistic pane-group swap converges on ack-driven emissions, and silencing the compound `run-shell`'s empty ack routed the first keystroke after a group-tab click to the *stale* pane — caught by the 7-regression group-click E2E, reproduced locally, and bisected to that exact hunk. A narrower variant (emit only while the settling suppression window is armed) was also insufficient. A test now pins the current behaviour. **Prerequisite for retrying:** marker-route the self-issued `list-panes`/`list-windows` commands the way captures are, so command completions can be classified exactly instead of by ack shape — then the emission decision can be made on identity rather than emptiness. Original finding follows.

   **Medium / bug — every unrecognized command response triggers a full state rebuild, and the list-panes sniffing heuristic can be spoofed.** `state.rs:1710-1721`: any `CommandResponse` that isn't marker-routed returns `state_changed: true, change_type: Full` — including the empty ack of every `send-keys` the frontend sends. Each one forces `to_state_update()`: a full `TmuxState` build, per-pane clone, and diff (Arc-sharing spares the grids, but windows/metadata are cloned and diffed per keystroke). Separately, `handle_command_response` (`state.rs:2015-2084`) treats any response line containing `%` and `,` with ≥11 comma-fields as list-panes output — output of an arbitrary `RunCommand` (which flows fire-and-forget through the same channel) matching that shape would create ghost panes in the aggregator. Recommendation: marker-wrap the self-issued `list-panes`/`list-windows` commands the same way captures are wrapped, then return `ChangeType::None` for unmarked acks.
