...
function checkDocsToScriptsConsistency() {
  ...
  const requiredCanonicalCommands = [
    'bash bin/agent-bootstrap',
    'npm run agent:fast-check',
    'npm run agent:full-check',
  ];
  ...
  for (const docPath of wrapperStrictDocs) {
    ...
    if (/npm run copilot:(?:bootstrap|fast-check|full-check)/.test(content)) {
      addError(`[docs] ${docPath} should use canonical agent:* wrappers, not copilot:* aliases.`);
    }
  }
}
