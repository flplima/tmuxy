...
/**
 * Assert container padding and inter-pane spacing are correct.
 *
 * Checks:
 * 1. Container padding: >= 8px between any pane edge and .pane-container edge
 * 2. Horizontal separator: ~1 charWidth between side-by-side panes
 * 3. Vertical separator: ~1 charHeight (PANE_HEADER_HEIGHT=21px) between stacked panes
 *
 * @param {Page} page - Playwright page
 */
async function assertSpacing(page) {
  ...
  const errors = await page.evaluate(() => {
    const errs = [];
    const snap = window.app?.getSnapshot();
    if (!snap?.context) return errs;
    const { charWidth, charHeight, panes, activeWindowId } = snap.context;

    const container = document.querySelector('.pane-container');
    if (!container) return ['No .pane-container found'];
    const cRect = container.getBoundingClientRect();

    // Only check panes in the active window that are within the visible container.
    // Filter by data-pane-id AND by checking the element is within the container bounds.
    ...
    // Filter out items that are clearly offscreen or layout-transitioning.
    // If an item extends significantly past the container on any side, skip it.
    const items = [];
    const rects = [];
    for (const el of allItems) {
      const r = el.getBoundingClientRect();
      // Skip if any edge extends more than 50px past the container
    ...
    if (rects.length === 0) return []; // Layout not settled — skip silently

    // Check 1: Container padding (>= 4px minimum).
    // The CSS padding is 8px, but pane centering may distribute the remaining space
    // unevenly. The minimum visible gap should be at least 4px (half the padding
    // when centering pushes panes to one edge).
    ...
    // Check 2 & 3: Inter-pane spacing
    if (rects.length >= 2) {
      const termContents = document.querySelectorAll('.terminal-content');
      const tRects = Array.from(termContents).map(el => el.getBoundingClientRect());
      ...
      for (let i = 0; i < tRects.length; i++) {
        for (let j = i + 1; j < tRects.length; j++) {
          ...
          // Check horizontal (side-by-side): overlapping Y ranges
          const yOverlap = Math.min(a.bottom, b.bottom) - Math.max(a.top, b.top);
          if (yOverlap > 10) {
            const hGap = Math.abs(
              a.right < b.left ? b.left - a.right : a.left - b.right
            );
            if (a.right < b.left || b.right < a.left) {
              const expected = charWidth;
              if (Math.abs(hGap - expected) > 2) {
                errs.push(
                  `H-gap between pane ${i} and ${j}: ${hGap.toFixed(1)}px (expected ~${expected.toFixed(1)}px = 1 charWidth)`
                );
              }
            }
          }

          // Check vertical (stacked): overlapping X ranges
          const xOverlap = Math.min(a.right, b.right) - Math.max(a.left, b.left);
          if (xOverlap > 10) {
            const vGap = Math.abs(
              a.bottom < b.top ? b.top - a.bottom : a.top - b.bottom
            );
            if (a.bottom < b.top || b.bottom < a.top) {
              const expected = charHeight; // = PANE_HEADER_HEIGHT = 21
              if (Math.abs(vGap - expected) > 2) {
                errs.push(
                  `V-gap between pane ${i} and ${j}: ${vGap.toFixed(1)}px (expected ~${expected.toFixed(1)}px = 1 charHeight)`
                );
              }
            }
          }
