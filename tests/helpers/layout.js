...
async function assertLayoutInvariants(page, options = {}) {
  ...
  const result = await page.evaluate(
    ({ checkCentering: cc, checkGaps: cg }) => {
      const errs = [];
      ...
      const gridFits =
        snap.context.totalWidth * charWidth <= cRect.width &&
      ...
      if (gridFits)
        for (let i = 0; i < rects.length; i++) {
          ...
          if (r.left < cRect.left - 2) {
            errs.push(
              `Overflow: pane ${ids[i]} left edge (${r.left.toFixed(1)}px) is outside container (${cRect.left.toFixed(1)}px)`,
            );
          }
          if (r.right > cRect.right + 2) {
            errs.push(
              `Overflow: pane ${ids[i]} right edge (${r.right.toFixed(1)}px) is outside container (${cRect.right.toFixed(1)}px)`,
            );
          }
          if (r.top < cRect.top - 2) {
            errs.push(
              `Overflow: pane ${ids[i]} top edge (${r.top.toFixed(1)}px) is outside container (${cRect.top.toFixed(1)}px)`,
            );
          }
          if (r.bottom > cRect.bottom + 2) {
            errs.push(
              `Overflow: pane ${ids[i]} bottom edge (${r.bottom.toFixed(1)}px) is outside container (${cRect.bottom.toFixed(1)}px)`,
            );
          }
        }
