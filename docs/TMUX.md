...
Rule: inside any `run-shell` string, write `##{...}` — run-shell's expansion halves it to `#{...}` for the inner command. `shquote` in `bin/tmuxy-cli` doubles every `#` it interpolates for this reason. (3.6b behaves the same; the bug had simply never been triggered through this path before.) `float-create` was the path that found the bug and no longer needs the escape: it stopped asking run-shell for the new pane's id (see below).
...
sees. `bind -n M-l tmuxy-nav-right` reads back through `list-keys` — the source
...
**Keyboard contract while a column has the keyboard.** Plain keys and non-binding chords stay in the column (the tree swallows them; the dock's pane receives them, Escape included). Prefix bindings and root bindings still run, and they act on the *tab grid* — tmux cannot make a chrome window current without blanking the tab, so `prefix %` with the dock focused splits the tab's active pane, not the dock. Leaving a column is Alt+h / Alt+l (or `l`/`q` in the tree), a click on a pane, or `tmuxy nav`.
...
tmux's marked pane (`select-pane -m`, cleared with `select-pane -M`; `prefix m` / `prefix M` by default) is carried on the wire as `#{pane_marked}` in the fixed tail of the monitor's `list-panes` format, so a mark set from any client — a plain `tmux` too — shows in every tmuxy client. The UI draws a flag in the pane header and the sidebar tree row and an outline on the pane (`--pane-marked-border`, themed). The pane context menu offers *Mark/Unmark Pane*, *Swap with Marked Pane* and *Join Marked Pane Here*, which run bare `swap-pane` / `join-pane`: without `-s`, tmux takes the marked pane as the source.
