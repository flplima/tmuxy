...
- `tabOverviewCols` — the config's `@tmuxy-tab-overview-cols` (default 3, capped at 12 by the backend), read with the appearance options: how many cards each row of the "all tabs" view holds. A phone-width window shows at most 2; the grid scrolls vertically when the rows outgrow the view
- `gestureFlags` — the config's `@tmuxy-gesture-swipe-tabs`, `@tmuxy-gesture-pinch-zoom` and `@tmuxy-gesture-pinch-overview` (all on by default), read with the appearance options. `gestureActor` reads them off the machine as each event arrives and claims no wheel or WebKit gesture event for a gesture that is off, so the browser keeps its own pinch-zoom and history swipe
- `gesture` — the trackpad gesture being drawn, owned by the `gestures` state. `gestureActor` turns wheel events (a slide, or a Chromium/Firefox pinch as ctrl+wheel) and WebKit's `gesturestart/change/end` (Safari, the desktop app) into `GESTURE_*` events. `GestureStage` writes each step as an inline `transform` on `.pane-layout` (or on the pinched pane) and nudges the cursor overlay: `transform` is not inherited and runs on the compositor, so a step invalidates no terminal styles and re-renders nothing; `PaneLayout` only re-renders when the gesture's shape changes. A slide moves the grid with the tab it pulls in drawn beside it from its real (normally hidden) panes. Releasing it past the mark switches tabs in that same step — the keyboard, the tab strip and tmux never wait for an animation — and because the tab switching in is already drawn where it will rest, the grid is simply put back by that offset and run home (`finishing`), with the tab just left sliding out beside it; released short, it slides back (`cancelling`). Neither ever reverses. What commits is where the slide would land carried on at the speed the fingers had (`swipeCommits`): half the pane width, so a slow drag short of that settles back while a flick switches; the finish then takes as long as that distance at that speed (`swipeSettleMs`, 90-320ms). Past either end of the strip the grid gives with rubber-band resistance. After a slide the actor ignores the momentum tail macOS keeps sending (it would otherwise start another slide), but a step bigger than that coast is the fingers back on the pad, so swiping twice quickly moves two tabs. A pinch in shrinks the grid (opening the "all tabs" view, or unzooming), a pinch out grows the pane under the fingers (zooming it), and a pinch out of the "all tabs" view grows the grid out of the current tab's card and enters that tab. A committed zoom holds the drawing (`handoff`) until `PaneLayout`'s zoom MutationObserver sees the new geometry land and animates on from the held box (`utils/zoomHandoff.ts`)
- Zoom animation — `PaneLayout` animates a pane zooming or unzooming from a MutationObserver on the pane boxes' inline style, not from a render diff: the zoomed flag can land a render after the geometry, so the observer catches the exact mutation, before paint, with the box the pane really had
...
That settled size is also what the grid must be POSITIONED against for the length of the slide. `PaneLayout` centres the grid in the pane area, and the container's width changes every frame while a column moves — so centring against the live width walked the pane area sideways for the whole animation, moving the panes' LEFT edge when the RIGHT column was the one opening. Both halves of the centring sum therefore come from the settled state while `sidebarMotion` is set: the width from `selectSettledPaneWidth` (the same one `beginSidebarMotion` tiles the grid for — one source of truth, so the grid cannot be tiled for one width and centred against another) and the grid's own width from `targetCols`, since tmux's re-tile lands somewhere mid-animation and an offset computed from the old column count steps when it arrives. The offset is then its final value from the first frame: the pane area slides with the divider and nothing else moves.
...
### Read-only view focus

On a read-only server (see [SECURITY.md](SECURITY.md#read-only-server)) nothing is optimistic, because nothing is sent. The store (`TmuxStore.dispatch`) splits ops in two: the focus ops — SelectWindow, SelectPane, Navigate — run through the same predictors and the tab and pane they land on become the model's `viewFocus`; every other op is refused before any patch is applied, and the refusal reaches the snackbar as a notice (keystrokes are refused silently). `viewFocus` is not an op: it has no reconciler and no expiry. It is laid over `derived` after the pending ops on every recompute — active window, the per-window active flags, active pane, and the grid extent of the viewed tab — while `committed` goes on recording whatever the writing client has active. It moves to the tab's own active pane when its pane closes and is dropped, back to following the server, when its tab does (`settleViewFocus` in `store/model.ts`).

Because the overlay lands in `derived`, everything downstream is unchanged: `SELECT_TAB`, `FOCUS_PANE`, tab navigation bindings, the float-per-tab filter and the keyboard actor's target all read the same fields they always did. The machine's own `readOnly` flag (from `CONNECTION_INFO`) covers the rest, in two layers:

- **Events.** Every event whose only purpose is to change the session carries the one `notReadOnly` guard (`machines/app/readOnlyGuard.ts`) — closing, zooming and writing to panes, tab create/close/reorder, floats and sidebars, pane-group tabs, drags and resizes, detach, the config-backed toggles — so it is dropped whole, before a drag preview or a flipped flag can show a change that will never happen. The writes nobody asked for (viewport reports, the dock's row count, the focus-request reply) check the flag inline. A request for tmux's copy mode opens the scroll view instead, which tells tmux nothing. Switching session stays allowed: it reopens the stream and sends no command.
- **Components.** The rule is that a viewer is not offered what it cannot do: a control for a refused action is not rendered (`useReadOnly()`), rather than rendered and refused. The "+" and close buttons, the pane menu and context menus, resize dividers, sidebar toggles and handles, the Tab Overview's "+" card, reorder drags, and the mutating sections of the app menu are all absent; a float cannot be dismissed (`Modal`'s `closable`); a parked pane-group member is listed but disabled, since its content is not on the wire. A sidebar is the writer's to open and close, so a viewer docks it when there is room and never draws it as an overlay it would have no way to dismiss (`selectSidebarLayout`). The "read-only" snackbar notice is therefore the fallback for what has no control to remove — a prefix binding, mostly.
