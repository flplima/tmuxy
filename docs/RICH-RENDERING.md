...
**1b. Row edges reach the pane border.** Each pane reaches half a separator column past its cells (`--pane-h-padding-left/right` on `.pane-content`), and a row painted edge to edge — a status line, an editor gutter — used to stop half a cell short of the border on both sides. Every row now paints that padding itself: `.terminal-line::before` takes the first cell's background and `::after` (which also fills whatever the row does not cover) the last cell's, published as `--row-edge-left/right` by both renderers (`rowEdgeBackground` in `terminalRendering.ts`; inverse video counts the way the cell renders). This is why `.terminal-container` and `.terminal-content` do not clip: `.pane-content` clips at its padding edge, and the padding has to stay paintable.
