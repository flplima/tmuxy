...
| **DCS Pq — Sixel** | `ESC P q … ESC \` | `control_mode/images.rs::try_parse_sixel` | same | Decoded by `icy_sixel`, re-encoded as PNG before serving. Only a DCS whose bytes before `q` are digits and `;` is Sixel: other DCS strings holding a `q`, such as nvim's XTGETTCAP query (`ESC P + q`), are not pictures |
...
An application inside tmux wraps these escapes in **DCS passthrough** (`ESC P tmux; <payload, every ESC doubled> ESC \`), which is how they reach a terminal sitting behind tmux at all. tmuxy is not behind tmux — it reads the pane stream over control mode — so the wrapper arrives here intact and the parser undoes it before anything else (`unwrap_passthrough`). Sending the escapes bare works too, but wrapping is better even here: tmux reads a lone APC string as a **pane title** (`input_exit_apc`), so an unwrapped Kitty frame turns the pane's title into base64 while it draws.

A picture is retired in one of four ways. A screen clear takes it away: `ESC [ 2 J`, or `ESC [ J` straight after the cursor goes home (how `clear` and curses applications empty the screen), and a full reset (`ESC c`). A screen switch sets it aside: entering the alternate screen (`?1049` / `?1047` / `?47`) hides the main screen's pictures, anything drawn there is dropped on leaving, and the main screen's come back — a placement is anchored to cells rather than drawn into them, so vt100 clearing its grid never touched it, and an image outlived the command or the editor that showed it. Kitty's `a=d` deletes placements (the default `d=a` and `d=A` selectors, which mean "all of them"; the narrower ones address images by a kitty id tmuxy does not keep, so they are left alone). And a new placement anchored at the same cell **replaces** the one already there, so a preview that homes the cursor and repaints once a second keeps exactly one frame on screen without stacking — and without the gap that deleting first would leave while the next frame crosses control mode. The frontend keys each `<img>` by that anchor rather than by the placement id, so a repaint reuses the element: the browser holds the previous picture until the new bytes have decoded, where building a fresh element painted nothing until they arrived and the preview blinked once a second.
...
**1b. Row edges reach the pane border.** Each pane reaches half a separator column past its cells (`--pane-h-padding-left/right` on `.pane-content`), and a row painted edge to edge — a status line, an editor gutter — used to stop half a cell short of the border on both sides. The rows cannot paint that padding themselves: the touch-scroll container they live in clips at its own box. `RowEdges` (`components/RowEdges.tsx`) sits beside that container inside the padded box and paints one strip per row in the padding columns — the first cell's background on the left, the last cell's on the right, bottom-anchored like the terminal. Colours come from `rowEdgeBackground` in `terminalRendering.ts`; inverse video counts the way the cell renders.

The right strip is painted only for a row that reaches the **last column**, and nothing is painted past a row's cells at all. A row's last cell is not its right edge: tmux reports a row as long as its content, so the last cell of a line of block art is a pixel of the picture, and taking it for an edge ran that colour to the border — an ASCII-art logo drew a bar across the pane, which no other terminal does (the cells beyond the content hold the default background). A status line does reach the last column, which is the case the padding was painted for.

**1c. Paint a cell's glyph into a cell.** Block Elements (`█ ▀ ▌ ▘ ░`) are drawn as CSS gradients rather than font outlines, because a font inks less than the cell box and stacked block art comes out striped (`components/blockGlyphs.ts`). The box they are painted into is a style-group span, which covers however many consecutive cells share a style — so the tile has to be sized in cells and repeated, not in percent. Sized in percent, a run of `▌` was one left half across the whole run and a quadrant covered half of it: every logo and bar built from half blocks came out stretched at exactly the width of the run that drew it.
...
The opposite mismatch also exists: a glyph that is **one** column to tmux but wider than one cell in the font. `⎿` (U+23BF, the elbow Claude Code draws before every tool result) is ~1.6 cells in FiraCode Nerd Font, and Nerd Font icons in the private-use area can be nearly 2. Grouped into a run, such a glyph laid the rest of the run out ~0.6 cell too long — the box stayed pinned, but the text inside painted past it and over the run that followed. `utils/glyphFit.ts` measures a symbol's advance against the font's cell once (a hidden probe with the real `.terminal-content` styles; the ratio is size-independent and cached per cell string, cleared when the webfont finishes loading), and a glyph that measures more than 1.15 cells gets its own 1-cell box with the glyph `scale()`d into it from the left edge (`.terminal-fit-glyph`). Only code points from U+2000 up are measured, so ordinary text costs one compare per cell. Story: `TerminalLine › FatNarrowGlyph`.
