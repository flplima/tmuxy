...
| `tests/13-ask-confirmation.test.js`    | `tmuxy ask` — sending keys to another pane once confirmed (confirmation overlay drawn over target pane, answered from originating pane, keys landing in target pane)                                                    |
| `tests/14-widget-pane-pointer.test.js` | Pointer handling over a pane embedding an iframe (`browser` widget) — click to activate pane, mouse drag across iframe boundary for resizing                                                                            |
...
- `cargo test --workspace` runs every crate's `#[cfg(test)]` modules plus the integration tests in `packages/tmuxy-core/tests/`. Most of the count is in `tmuxy-core`; `tmuxy-server`, `tmuxy-tauri-app`, `tmuxy-connect`, `tmuxy-wasm` and `tmuxy-tree` have smaller suites
...
| Uncovered crates and scripts                      | `packages/tmuxy-demo` has no tests and `deploy-demo.yml` deploys without testing; the `tests/qa-*.js` scripts and `tests/tauri/trace-demo.js` are run by no job.                                                                                                                                                                         |
